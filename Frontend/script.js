...
document.getElementById('register-form').addEventListener('submit', async function (e) {
    ...
    if (name && email && password) {
        try {
            const response = await fetch('http://localhost:5192/api/Customer', {
                method: 'POST',
...
async function loginCustomer(email, password) {
    try {
        const response = await fetch('http://localhost:5192/api/Customer/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
document.getElementById('delete-order-form').addEventListener('submit', async function (e) {
    ...
    if (orderId && currentCustomer) {
        try {
            const response = await fetch(`http://localhost:5192/api/Order/${orderId}/customer/${currentCustomer.id}`, {
                method: 'DELETE',
...
async function fetchOrders() {
    try {
        const response = await fetch(`http://localhost:5192/api/Order/customer/${currentCustomer.id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
async function checkBanStatus() {
    ...
    try {
        const response = await fetch(`http://localhost:5192/api/Customer/ban-status`, {
            headers: { Authorization: `Bearer ${token}` }
        });
